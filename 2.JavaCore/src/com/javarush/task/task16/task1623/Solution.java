...
public class Solution {
    ...
    public static class GenerateThread extends Thread {
        ...
        public void run(){ // тут зарыта собака, если сделать цикл или просто создать новый объект этого же класса здесь
            // произойдет бесконечное создание новых потоков, поэтому нужно сразу (проще всего через if) ограничить создание новых потоков
            // тут нужно понять что каждый новый поток тут же создает еще один, а тот в свою очередь еще один и так дол бесконечеости
            // поэтому нам и нужно createdThreadCount, которое volatile, т.е. одно на все потоки

            if (createdThreadCount<Solution.count){
                System.out.println(new GenerateThread()); // вот тут тонкий момент для валидатора, нужно именно в
                // System.out.println прописовать создание нового объекта, если создать отдельно объект о отделно проаисать this
